...
class SeaIceModel(error_norms):
    ...
    def plot_temperature(self, z_depth, savefig=True, Buffo_matlab=False):
        ...
        if Buffo_matlab is True:
            ax1.plot(x_axis_iter*seaicemodel_obj.dt/3600,df_depth[:24999], 'b',alpha=0.2,label='Buffo-matlab' )
        ax1.set_xlabel("t in hours")
...
if __name__ == "__main__":
    ...
    run_time = end-start
    seaicemodel_obj.error_analytical_numerical()
    seaicemodel_obj.plot_error_temp(5000, norm='inf', savefig=True)     # norm="inf" OR norm="2" OR norm="1"
    seaicemodel_obj.plot_error_temp_diff(5000, savefig=True)
    seaicemodel_obj.plot_depth_over_time()
    seaicemodel_obj.plot_temperature(z_depth=0.1, savefig=True)
